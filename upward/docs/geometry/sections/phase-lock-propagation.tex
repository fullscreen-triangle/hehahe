...
\begin{figure*}[htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/dynamics_simulation_single_cell.png}
    \caption{
...
\begin{figure*}[htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/Figure21_Phase_Locking_Mechanism.png}
    \caption{
    \textbf{Phase-locking mechanism: Why thermodynamics alone cannot predict binding affinity.}
    \textbf{(Panel A)} Phase-locking constrains the energy landscape showing comparison between Thermodynamic View (left, blue box) and Categorical Reality (right, red box). Thermodynamic View: Free Energy Landscape (blue curve, smooth sinusoidal oscillations with amplitude $\sim 0.5$ arbitrary units, two peaks and one trough over domain). Categorical Reality: Phase-Locked Landscape (red curve, flattened oscillations with amplitude $\sim 0.2$ arbitrary units, dashed gray reference lines showing original unconstrained landscape). Purple double-headed arrow labeled ``Phase-Lock Energy Cost'' connects the two landscapes. Red arrow labeled ``Correction'' points from categorical back to thermodynamic. Title annotation: ``Phase-Locking: Why Energy is `Lost'\,''. The phase-locking flattens the energy landscape by $\sim 60\%$, converting oscillatory degrees of freedom into categorical constraints. This ``lost'' energy ($\Delta E_{\text{lock}}$) is not dissipated but locked into maintaining phase relationships between oscillators, creating an effective energy barrier that thermodynamic $\Delta G$ calculations miss.
    \textbf{(Panel B)} Thermodynamics vs. categorical mechanics showing two contrasting approaches. Left (blue box): Thermodynamic Approach with equation $\Delta G = \Delta H - T\Delta S$ (black text), annotation ``Ignores microscopic states'' (red text). Diagram below: Single Average State (single teal oval labeled ``1''). Right (red box): Categorical Approach with text ``Track ALL interactions'' (red text), ``Accounts for phase-locking'' (red text).
    \textbf{(Panel C)} Multiple paths to similar affinity showing three molecular complexes with nearly identical binding constants despite different interaction compositions. Title: ``Different Interaction Combinations $\rightarrow$ Similar Binding Constants''. Left column: O complex (red oval at top labeled ``O'', red box below listing interactions): coordination, electrostatic, H-bond, dipole, vdW, London, water network (seven interaction types, red text). Bottom red box: $K = 1.34 \times 10^{11}$ (binding constant). Center column: CO complex (blue oval at top labeled ``CO'', blue box below): covalent, electrostatic, H-bond, dipole, vdW, London, water network (seven interaction types, blue text, note ``covalent'' replaces ``coordination''). Bottom blue box: $K = 1.49 \times 10^{11}$. Right column: CN complex (teal oval at top labeled ``CN'', teal box below): coordination, pi-bonding, electrostatic, H-bond, dipole, vdW, water network (seven interaction types, teal text, note ``pi-bonding'' replaces ``covalent''). Bottom teal box: $K = 9.67 \times 10^{12}$. Yellow annotation box at bottom: ``Similar $K$ values despite different interaction compositions!
    \textbf{(Panel D)} Fundamental limitations of thermodynamic predictions showing four numbered limitations with explanations.
    }
