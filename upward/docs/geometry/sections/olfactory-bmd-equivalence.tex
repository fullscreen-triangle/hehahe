...
\begin{figure*}[htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/molecular_binding_analysis.png}
    \caption{
    \textbf{Molecular binding analysis: Affinity, coupling, conformational change, and residence time across five drugs.}
    \textbf{(Top left)} Binding affinity distribution showing histogram of $\log_{10}(K_d)$ [nM] (x-axis, $2.0$ to $3.4$) with frequency (y-axis, $0$ to $3.0$). Distribution is bimodal: first peak at $\log_{10}(K_d) = 2.6$ (frequency $\sim 3.0$, blue bars), second peak at $\log_{10}(K_d) = 3.0$ (frequency $\sim 3.0$, light blue bars). Red dashed vertical line at $\log_{10}(K_d) = 2.0$ (annotation: ``$100~\text{nM}$ threshold''). Most drugs cluster above threshold ($K_d > 100~\text{nM}$, moderate-to-weak binding). The bimodal distribution suggests two binding classes: high-affinity ($K_d \sim 400~\text{nM}$) and low-affinity ($K_d \sim 1000~\text{nM}$).
    \textbf{(Top center)} Coupling vs. binding affinity showing oscillatory coupling strength (y-axis, $0.1$ to $0.6$) vs. $\log_{10}(K_d)$ [nM] (x-axis, $2.6$ to $3.4$). Blue scatter points show negative correlation: higher affinity (lower $K_d$, left) correlates with stronger coupling ($\sim 0.6$), lower affinity (higher $K_d$, right) correlates with weaker coupling ($\sim 0.1$). The inverse relationship validates oscillatory coupling mechanism---drugs that phase-lock strongly to target oscillations (high coupling) achieve tighter binding (low $K_d$).
    \textbf{(Top right)} Conformational change distribution showing histogram of conformational change amplitude (x-axis, $0.05$ to $0.35$) with frequency (y-axis, $0$ to $5$). Distribution peaks at $0.10$ (frequency $= 5$, green bar, largest), secondary peak at $0.05$ (frequency $= 4$), tertiary peak at $0.15$ (frequency $= 2$). Long tail extends to $0.35$ (frequency $= 1$). The right-skewed distribution indicates most drugs induce small conformational changes ($< 0.15$), with few inducing large changes ($> 0.25$)---consistent with ``induced fit'' model where binding optimizes pre-existing conformations rather than creating new ones.
    \textbf{(Bottom left)} Binding affinity heatmap showing $-\log_{10}(K_d)$ (color scale $-3.4$ to $-2.6$, light red to dark red) for five drugs (rows: aspirin, aripiprazole, lithium, atorvastatin, citalopram) across five targets (columns: htr2a\_gpcr, drd2\_gpcr, inpp1\_enzyme, cyp2d6\_enzyme, gsk3b\_kinase). Aspirin: moderate affinity across all targets ($-\log_{10}(K_d) \sim -2.6$ to $-3.0$, orange-red). Aripiprazole: strong affinity for GPCRs ($-2.6$ to $-2.8$, dark red), weaker for enzymes ($-3.0$, red). Lithium: uniform moderate affinity ($-3.0$ to $-3.2$, light red). Atorvastatin: strong affinity for enzymes ($-2.6$ to $-2.8$, dark red), weaker for GPCRs ($-3.2$, light red). Citalopram: strong affinity for htr2a ($-2.6$, dark red), moderate for others ($-3.0$ to $-3.4$, red-light red). The heatmap reveals target selectivity: aripiprazole and citalopram prefer GPCRs (serotonin/dopamine receptors), atorvastatin prefers enzymes (metabolic targets), lithium shows no selectivity (mood stabilizer).
    \textbf{(Bottom center)} Drug residence times showing residence time [seconds] (y-axis, log scale $10^{-1}$ to $10^0$) for five drugs (x-axis). Aspirin: $\sim 10^{-1}~\text{s}$ (box plot: median orange line, IQR $0.08$--$0.12~\text{s}$, outlier at $0.05~\text{s}$). Aripiprazole: $\sim 10^0~\text{s}$ (median $0.5~\text{s}$, IQR $0.3$--$0.7~\text{s}$). Lithium: $\sim 10^{-1}~\text{s}$ (median $0.03~\text{s}$, IQR $0.02$--$0.05~\text{s}$, shortest residence). Atorvastatin: $\sim 10^0~\text{s}$ (median $0.6~\text{s}$, IQR $0.4$--$0.8~\text{s}$, longest residence). Citalopram: $\sim 10^0~\text{s}$ (median $0.4~\text{s}$, IQR $0.3$--$0.5~\text{s}$). The 10$\times$ range demonstrates that residence time varies independently of affinity---atorvastatin has longest residence ($\sim 0.6~\text{s}$) despite moderate affinity ($K_d \sim 1000~\text{nM}$), while lithium has shortest residence ($\sim 0.03~\text{s}$) despite similar affinity. This validates the kinetic selectivity hypothesis: therapeutic efficacy depends on residence time (duration of target engagement) as much as affinity (strength of binding).
    \textbf{(Bottom right)} Binding prediction confidence showing histogram of prediction confidence (x-axis, $0.70$ to $0.95$) with frequency (y-axis, $0$ to $20$). Distribution is strongly right-skewed: peak at confidence $= 0.95$ (frequency $= 25$, yellow bar, annotation: ``Good Confidence''). Secondary peak at confidence $= 0.90$ (frequency $= 2$). Red dashed vertical line at confidence $= 0.70$ (threshold). Most predictions exceed $0.90$ confidence ($> 80\%$ of data), indicating high reliability. The high confidence validates the oscillatory coupling model---binding predictions based on phase-locking strength are robust and reproducible.
    }
...
\begin{figure*}[htbp]
    ...
    \caption{
    \textbf{Olfactory ensemble geometry analysis: Hole radius, density, and spatial distribution for 50 BMDs.}
    \textbf{(Panel A)} Hole radius distribution showing histogram of hole radius [\AA] (x-axis, $2.0$ to $5.0$) with count (y-axis, $0$ to $7$). Distribution is multimodal: peak at $2.5~\text{\AA}$ (count $= 7$, purple bar, highest), secondary peak at $3.0~\text{\AA}$ (count $= 6$, blue bar), tertiary peak at $4.0~\text{\AA}$ (count $= 6$, teal bar), quaternary peak at $4.5~\text{\AA}$ (count $= 5$, yellow bar). Red dashed vertical line at mean $= 3.46~\text{\AA}$ (annotation: ``Mean: $3.46~\text{\AA}$''). The broad distribution ($2.0$--$5.0~\text{\AA}$, 2.5$\times$ range) indicates olfactory holes span multiple size classes, enabling discrimination of odorants with different molecular volumes.
    \textbf{(Panel B)} Density vs. radius showing molecular density (y-axis, $0.00$ to $0.14$, left axis; color bar $0.00$ to $0.14$, right axis) vs. hole radius [\AA] (x-axis, $2.0$ to $5.0$). Scatter points color-coded by density (purple to yellow). Red dashed line shows linear fit with negative slope (annotation: ``$r = -0.881$, $p = 3.014 \times 10^{-17}$''). Yellow box annotation: ``Linear fit''. Density decreases from $0.10$ at $2.5~\text{\AA}$ to $0.01$ at $5.0~\text{\AA}$. The strong negative correlation ($r = -0.881$, $p < 10^{-16}$) demonstrates that larger holes have lower molecular density---consistent with inverse-square law for surface-to-volume ratio. Small holes ($< 3~\text{\AA}$) pack molecules tightly (high density $> 0.08$), large holes ($> 4~\text{\AA}$) have sparse packing (low density $< 0.02$).
    \textbf{(Panel C)} Hole center positions showing 3D scatter plot with axes $X$ [\AA] ($-20$ to $+20$), $Y$ [\AA] ($-20$ to $+20$), $Z$ [\AA] ($-20$ to $+20$). Spheres color-coded by radius [\AA] ($2.5$ to $4.5$, blue to red). Most holes cluster within $\pm 10~\text{\AA}$ cube (central region, $\sim 40$ holes). Few outliers extend to $\pm 20~\text{\AA}$ (periphery, $\sim 10$ holes). Color gradient shows no spatial segregation by size---small (blue) and large (red) holes intermix uniformly. The isotropic distribution indicates olfactory holes are randomly positioned within $\sim 20~\text{\AA}$ radius sphere, consistent with stochastic receptor placement on olfactory neuron membrane.
    \textbf{(Panel D)} Geometry evolution showing dual y-axis plot: hole radius [\AA] (left y-axis, $2.0$ to $5.0$, blue circles) and molecular density (right y-axis, $0.02$ to $0.14$, red squares) vs. geometry index (x-axis, $0$ to $50$). Blue line (hole radius) oscillates between $2.0~\text{\AA}$ and $5.0~\text{\AA}$ with period $\sim 10$ indices. Red line (molecular density) anti-correlates with radius: peaks at $0.14$ when radius $= 2.0~\text{\AA}$ (indices $5$, $10$), troughs at $0.02$ when radius $= 5.0~\text{\AA}$ (indices $0$, $15$, $30$). Annotation: ``Hole Radius'' (blue legend), ``Molecular Density'' (red legend). The anti-phase oscillations confirm inverse relationship: as hole expands (radius increases), density drops (molecules spread out); as hole contracts (radius decreases), density rises (molecules compress). The $\sim 10$-index periodicity suggests olfactory ensemble cycles through size classes on timescale $\sim 10~\text{ms}$ (assuming $1~\text{ms/index}$), enabling temporal multiplexing of odorant discrimination.
    }
...
\begin{figure*}[htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/olfactory_spatial_analysis.png}
    \caption{
    \textbf{Olfactory ensemble spatial analysis: 3D distribution, projections, and radial statistics for 50 BMDs at $[\text{O}^-] = 0.005$.}
    \textbf{(Panel A)} 3D O$^-$ distribution showing 3D scatter plot with axes $X$ [\AA] ($-30$ to $+30$), $Y$ [\AA] ($-30$ to $+30$), $Z$ [\AA] ($-20$ to $+20$). Spheres color-coded by distance from COM [\AA] ($5$ to $30$, purple to yellow). Dense cluster at center ($0$, $0$, $0$, purple-blue, $\sim 100$ molecules, distance $< 10~\text{\AA}$). Sparse shell at periphery ($\pm 20~\text{\AA}$, yellow-orange, $\sim 50$ molecules, distance $> 20~\text{\AA}$). Red star marks COM at origin. The radial gradient (purple center, yellow periphery) demonstrates spherical symmetry with $\sim 10~\text{\AA}$ core and $\sim 30~\text{\AA}$ outer radius.
    \textbf{(Panel B)} XY projection showing 2D scatter plot with axes $X$ [\AA] ($-30$ to $+30$), $Y$ [\AA] ($-30$ to $+30$). Spheres color-coded by distance [\AA] ($5$ to $30$, purple to yellow). Dense central cluster ($-5$ to $+5~\text{\AA}$, purple-blue, $\sim 80$ molecules). Diffuse outer ring ($\pm 20~\text{\AA}$, orange-yellow, $\sim 70$ molecules). The XY projection reveals anisotropy: molecules preferentially occupy four quadrants ($\pm 10~\text{\AA}$, $\pm 10~\text{\AA}$) rather than uniform disk, suggesting tetrahedral coordination geometry.
    \textbf{(Panel C)} Radial distribution showing histogram of distance from COM [\AA] (x-axis, $5$ to $30$) with count (y-axis, $0$ to $16$). Distribution peaks at $15~\text{\AA}$ (count $= 16$, blue bar, highest). Red dashed vertical line at mean $= 16.0~\text{\AA}$ (annotation: ``Mean: $16.0~\text{\AA}$''). Orange dashed vertical line at median $= 15.7~\text{\AA}$ (annotation: ``Median: $15.7~\text{\AA}$''). Secondary peaks at $10~\text{\AA}$ (count $= 13$) and $20~\text{\AA}$ (count $= 13$). The Gaussian-like distribution with mean $\approx$ median indicates symmetric radial packing. The $15~\text{\AA}$ peak corresponds to first coordination shell around COM, consistent with close-packed sphere model.
    \textbf{(Panel D)} Coordinate distributions showing histogram of position [\AA] (x-axis, $-30$ to $+30$) with count (y-axis, $0$ to $35$) for three coordinates: $X$ (red bars), $Y$ (teal bars), $Z$ (blue bars). All three distributions peak at $0~\text{\AA}$ (central bin): $X$ count $= 32$, $Y$ count $= 35$ (highest), $Z$ count $= 26$. Distributions are symmetric around zero with width $\sim 20~\text{\AA}$ (FWHM). $Y$ distribution is narrowest (most peaked), $Z$ distribution is broadest (most spread). The triple-Gaussian form confirms isotropic packing in $XY$ plane with slight compression along $Z$ axis---consistent with oblate spheroid geometry (pancake-shaped ensemble).
    }
    \label{fig:olfactory_spatial}
\end{figure*}
