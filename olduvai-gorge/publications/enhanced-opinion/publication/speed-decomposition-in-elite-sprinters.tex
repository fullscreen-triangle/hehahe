...
\begin{enumerate}[leftmargin=1.2em]
\item Spinal and brainstem reflex arcs (H-reflex, stretch reflex, vestibulo-motor coupling) \cite{sherrington1925integrative,eccles1957physiology}
\item Alpha motor neuron recruitment and rate coding \cite{henneman1957relation}
\item Neuromuscular junction transmission \cite{katz1967nerve,katz1972remarks}
\item Contractile apparatus force generation (sliding filament dynamics) \cite{hodgkin1952quantitative,enoka2015neuromechanics}
\item Proprioceptive feedback loops closed in the spinal cord \cite{kandel2013principles,purves2018neuroscience}
\end{enumerate}
...
\begin{enumerate}[leftmargin=1.2em]
\item Perceptual integration of competitor position and speed \cite{zeki1993vision}
\item Threat-density evaluation from visual field and proprioceptive context \cite{koch2004quest}
\item Nervous-system calibration to field coherence \cite{critchley2005neural}
\item Conscious and preconscious decision-making about effort allocation \cite{kahneman2011thinking}
\item Motor planning and trajectory selection under uncertainty \cite{shadmehr2005biology,kawato1999internal,wolpert2000principles,todorov2002finding}
\item Interoceptive integration (lactate sensing, breathing patterns, autonomic state) \cite{saper2002brain}
\end{enumerate}
...
The spinal stretch reflex operates at latencies of 20--30\,ms \cite{sherrington1925integrative,eccles1957physiology}. This is the minimum possible latency for a monosynaptic reflex arc: sensory receptor → sensory neuron → spinal interneuron → motor neuron → muscle $\approx 20$\,ms. Polysynaptic pathways add latency; the Ia inhibitory reflex (Renshaw inhibition) adds 2--3\,ms. The vestibulo-ocular reflex, which must stabilize gaze during head rotation, operates at 15--20\,ms \cite{kandel2013principles}.

For sprint running, the relevant reflex latencies are \cite{enoka2015neuromechanics,winter2009biomechanics,novacheck1998biomechanics}:
\begin{itemize}[leftmargin=1.2em]
...
The critical constraint is the ground-contact cycle \cite{cavagna1977mechanical,kram1990energetics,bolt2008}. A sprinter at maximum velocity ($v \approx 10$ m/s) contacts the ground roughly every 100\,ms. Within that 100\,ms contact window, the neuromuscular system must:
\begin{enumerate}[leftmargin=1.2em]
...
Conscious perception and decision-making operate at vastly slower timescales \cite{kahneman2011thinking,koch2004quest}. The relevant latencies are:

\begin{itemize}[leftmargin=1.2em]
\item \textbf{Visual perception of motion}: 80--150\,ms (time to detect stimulus onset and direction) \cite{zeki1993vision}
\item \textbf{Spatial perception}: 150--200\,ms (integrate visual information into body-relative frame)
\item \textbf{Conscious decision or categorization}: 200--500\,ms (identify threat, choose action) \cite{card1983psychology,nielsen1993response}
\item \textbf{Motor planning}: 200--300\,ms (prepare reaching or stepping movement) \cite{shadmehr2005biology,kawato1999internal}
...
\begin{proof}
Consider a unified controller $u(t) = \pi(\text{state}(t - \tauc), \theta)$ operating on a plant with reflex latency $\taur$ \cite{astrom2010feedback}. The closed-loop system is
\begin{align}
\dot{x}(t) &= f(x(t), u(t), w(t)) \\
u(t) &= \pi(x(t - \tauc), \theta)
\end{align}
The phase lag introduced at frequency $\omega$ is $\omega(\tauc)$. For stability near a limit cycle (sprint at constant speed), the crossover frequency must satisfy $\omega_c \tauc < \pi/2 - \phi_m$ where $\phi_m$ is the required phase margin (typically $\sim 30°$) \cite{astrom2010feedback}.
...
\item The cognitive loop does not directly command muscles; it modulates parameters or set-points available to the reflex loop \cite{feldman1966control,feldman1986once,bizzi1992reaching}
\end{enumerate}
This is not a design choice; it is a thermodynamic consequence of latency separation. Clinical natural experiments support the decoupling: subjects with chronic proprioceptive deafferentation (e.g.~the Ian Waterman case) lose the fast loop yet retain the slow loop, and the resulting movement requires constant visual guidance at cognitive-loop latencies---confirming that the two loops are physically distinct \cite{cole1991study,cole1995knowledge,cole2016body,rothwell1982loss,sainburg1993sensorimotor,sainburg1995loss,sainburg1999proprioceptive,ghez1995reaching,gordon1995reaching,taub1965movement,taub1966primate}.
\end{corollary}
...
The reflex substrate converts a desired trajectory (produced by cognitive substrate) into muscular force and ground reaction \cite{enoka2015neuromechanics,winter2009biomechanics}. Given a set-point or trajectory $\text{traj}_{\text{desired}}(t)$ from the cognitive system, the reflex system produces:
\begin{equation}
...
The cognitive substrate observes the field, integrates perceptual information, and decides what trajectory to attempt \cite{shadmehr2005biology,kahneman2011thinking,todorov2002finding}:
\begin{equation}
...
To reach elite levels requires \cite{schmidt2019motor}:
\begin{enumerate}[leftmargin=1.2em]
\item Extreme genetic endowment (top 0.01\% of population in sprint-relevant traits)
\item Decade-scale training (10,000+ hours of deliberate practice) \cite{schmidt2019motor}
\item Optimal technical execution (learned through thousands of repetitions) \cite{bolt2008,novacheck1998biomechanics}
\item Optimal nutrition and recovery (professional-scale resources) \cite{weir1949new,mifflin1990new,harris1919basal,levine2005measurement}
\item Psychological resilience and moment calibration (innate + trained) \cite{kahneman2011thinking,lutz2004attention}
\end{enumerate}
...
\begin{proof}
Consider the production function for sprint speed \cite{cavagna1977mechanical,kram1990energetics,bolt2008}:
\begin{equation}
v_{\text{max}} = f(\text{ACSA}, \text{fiber-type}, \text{neural-drive}, \text{O2-delivery}, \text{technique})
\end{equation}

Each parameter has a genetic ceiling \cite{enoka2015neuromechanics}:
\begin{itemize}[leftmargin=1.2em]
...
The 2026 Enhanced Games experiment provides an almost perfect null test of the pharmacology hypothesis \cite{enhanced2026official}: maximum reflex enhancement (documented PES access for all competitors), minimal field coherence (weak competitors). If pharmacology were the limiting factor, this should produce historic times. It did not.
...
\item \textbf{Era comparison}: The 1988--2000 period (widespread unreported PES use) saw improvements in world records of $\sim 0.01$--$0.03$ s/year. The 2000--2026 period (more testing, less widespread PES) saw similar rates of improvement \cite{bolt2008}. No acceleration of improvement when PES became more available.
...
\begin{enumerate}[leftmargin=1.2em]
\item \textbf{Prediction 1}: Field coherence variations should explain more sprint-time variance than pharmacological status. **Test**: In a cohort of elite sprinters over multiple seasons, regress times against (a) field coherence (sum of competitor times) and (b) documented or estimated pharmacological status. Field coherence should have larger coefficient. Computational tracking of competitor positions and action sequences from broadcast video is now feasible at scale \cite{fernandez2018wider,decroos2019actions,theagarajan2021tracklet,dorazio2009markov}, making this regression tractable.
...
\begin{itemize}[leftmargin=1.2em]
\item \textbf{Chess}: A grandmaster's cognitive substrate (calculation depth, threat perception) is the binding constraint \cite{kahneman2011thinking}. Stimulant enhancement (e.g., caffeine, modafinil) may improve alertness marginally, but does not improve chess ability. The chess engine effect (calculating 10 moves ahead) cannot be enhanced pharmacologically because it is not a physiological trait—it is a learned cognitive structure.

\item \textbf{Professional driving}: The reflex substrate (vehicle control, line precision) operates at 20--50 Hz. The cognitive substrate (threat planning, terrain reading) operates at 1 Hz \cite{kelly2008controlvehicle,limebeer2015optimal}. A Formula 1 driver's lap time is determined primarily by cognitive factors (braking point selection, line choice, reading competitors), not by muscle strength or reaction time \cite{milliken1995race,wright2001formula}. Pharmacological stimulation does not improve driving performance because the binding constraint is cognitive, not physiological.

\item \textbf{Surgical performance}: An elite surgeon's reflex substrate (hand steadiness, motor precision) is already optimized through 10,000+ hours of training \cite{schmidt2019motor}. Cognitive substrate (decision-making, threat anticipation, moment awareness) is the binding constraint. A stimulant might marginally increase alertness but does not improve surgical decision-making or moment calibration.
\end{itemize}
...
Records compiled from \cite{bolt2008,paris2024olympics,enhanced2026official} and IAAF/World Athletics official archives.
...
\textbf{Reflex latency estimate:} The monosynaptic stretch reflex latency is measured at 20--30\,ms by standard electrophysiology \cite{sherrington1925integrative,eccles1957physiology,hodgkin1952quantitative}. Adding proprioceptive signal transduction (receptor → sensory neuron soma) adds $\sim 5$\,ms. Adding motor neuron conduction adds $\sim 5$\,ms. Total: 30--40\,ms. Adding polysynaptic pathways and muscle force latency: 40--60\,ms \cite{enoka2015neuromechanics,kandel2013principles}. Elite sprinters operate at the fast end of this range through training; we estimate $\taur \approx 50$\,ms as a reasonable upper bound for reflex latency in running.

\textbf{Cognitive latency estimate:} Visual stimulus detection latency is 80--150\,ms \cite{zeki1993vision,koch2004quest}. Spatial integration adds 50--100\,ms. Decision categorization adds 100--200\,ms \cite{kahneman2011thinking}. Total conscious cycle: 300--500\,ms. Studies of reaction time (detecting a simple stimulus and pressing a button) show minimum latencies of 200--250\,ms \cite{card1983psychology,nielsen1993response}. Complex decisions involving threat evaluation and trajectory selection add another 200--300\,ms \cite{shadmehr2005biology,wolpert2000principles}. We estimate $\tauc \approx 500$\,ms for a full cognitive cycle in a sprint.
...
Mechanisms below are summarised from standard neurophysiology and exercise-physiology references \cite{enoka2015neuromechanics,kandel2013principles,purves2018neuroscience}.

\textbf{Anabolic steroids:}
\begin{itemize}[leftmargin=1.2em]
\item Bind androgen receptors on muscle cells
\item Increase protein synthesis
\item Increase type II muscle fiber cross-sectional area \cite{enoka2015neuromechanics}
...
\item Increase oxygen-carrying capacity \cite{weir1949new,levine2005measurement}
...
\item Increase neural drive and aggressiveness \cite{henneman1957relation}
...
\begin{itemize}[leftmargin=1.2em]
\item Increase catecholamine (dopamine, norepinephrine) levels \cite{saper2002brain,critchley2005neural}
\item Increase alertness and wakefulness
\item Increase motor cortex excitability and alpha motor neuron recruitment \cite{henneman1957relation,katz1967nerve,katz1972remarks}
\item Effect on cognitive substrate: Marginal (increased arousal); not improved decision-making accuracy or threat perception \cite{kahneman2011thinking}
\end{itemize}
